 ...
 *
 * If only --incomingdir is provided, ignored/completed are inferred as siblings:
 *   ignoreddir   = dirname( incomingdir ) . '/ignored'
 *   completeddir = dirname( incomingdir ) . '/completed'
 *
 ...
 * Usage examples:
 *
 * 1) Minimal, infer siblings (ignored/completed) from incoming:
 *    php SFTPDownload.php \
 *      --incomingdir /var/spool/audit/paypal/incoming \
 *      --connectstring user@host \
 *      --passwordfile /secure/pw.txt \
 *      --remotedir /reports
 *
 * 2) Limit to N most recent eligible files (by remote mtime), optionally filter:
 *    php SFTPDownload.php \
 *      --incomingdir /var/spool/audit/paypal/incoming \
 *      --connectstring user@host \
 *      --passwordfile /secure/pw.txt \
 ...
 *      --filter STL
 *
 * 3) With optional remote delete (only if remote mtime is old enough):
 *    php SFTPDownload.php \
 *      --incomingdir /var/spool/audit/paypal/incoming \
 *      --connectstring user@host \
 *      --passwordfile /secure/pw.txt \
 *      --remotedir /reports \
 *      --delete-remote \
 *      --delete-remote-age-days 7
 *
 * 4) Use a config name (looks in /etc/fundraising/<name>.yaml or $HOME/.fundraising/<name>.yaml):
 *    php SFTPDownload.php --configname dlocal-audit
 *
 * 5) dry run mode
 *    php vendor/wikimedia/smash-pig/Maintenance/SFTPDownload.php \
 *      --incomingdir /var/spool/audit/paypal/incoming \
 *      --connectstring user@host \
 *      --passwordfile /etc/my.pw \
 *      --limit 1 \
 *      --dry-run 1 \
 ...
 * - Host key pinning is supported via config sftp.host_key (ssh-rsa <base64>).
...
class SFTPDownload extends MaintenanceBase {
	...
	public function __construct() {
		...
		// Local paths
		$this->addOption( 'incomingdir', 'Local incoming directory (download destination)', null, 'i' );
		$this->addOption( 'ignoreddir', 'Local ignored directory (default: sibling of incomingdir)', null, 'g' );
		$this->addOption( 'completeddir', 'Local completed directory (default: sibling of incomingdir)', null, 'c' );
		...
		$this->addOption( 'config', 'Explicit path to config yaml', null );

		// Remote connection
		$this->addOption( 'connectstring', 'SFTP connect string (user@host)', null, 's' );
		$this->addOption( 'passwordfile', 'File containing SFTP password', null, 'p' );
		$this->addOption( 'private-key-file', 'Path to private key file (PEM/OpenSSH). Overrides config.', null );
		$this->addOption( 'private-key-password', 'Passphrase for private key file (if encrypted)', null );
		$this->addOption( 'remotedir', 'Remote directory to download from (or sftp.remote_root in config)', null, 'r' );
	...
	public function execute() {
		...
		$incomingOptOrConfig = $this->chooseOptionOrConfig( 'incomingdir', $this->config, 'incoming_path', '' );
		if ( !is_string( $incomingOptOrConfig ) || trim( $incomingOptOrConfig ) === '' ) {
			$this->error( 'incomingdir is required (or set incoming_path in config).' );
		}

		$incomingDir = $this->normalizePath( $incomingOptOrConfig );
		$incomingDir = $this->maybeRealpath( $incomingDir );

		$ignoredDir = $this->normalizePath( (string)$this->getOption( 'ignoreddir' ) );
		$completedDir = $this->normalizePath( (string)$this->getOption( 'completeddir' ) );
		...
		$remoteDirOptOrConfig = $this->chooseOptionOrConfig( 'remotedir', $config, 'sftp.remote_root', '' );
		if ( !is_string( $remoteDirOptOrConfig ) || trim( $remoteDirOptOrConfig ) === '' ) {
			$this->error( 'remotedir is required (or set sftp.remote_root in config).' );
		}
		$remoteDir = rtrim( $remoteDirOptOrConfig, '/' );

		$connectString = (string)$this->getOption( 'connectstring' );
		if ( $connectString === '' ) {
			...
			if ( !is_string( $host ) || trim( $host ) === '' || !is_string( $username ) || trim( $username ) === '' ) {
				$this->error( 'connectstring is required (or set sftp.host and sftp.username in config).' );
			}
		...
		$password = $this->readPasswordFromConfigOrFile( $config );
		...
		// Broad "discovery mode" for legacy servers.
		$this->configureSshAlgorithmDiscovery( $sftp );

		$this->verifyPinnedHostKeyOrError( $sftp, $config, $host );
		...
		if ( $password !== null ) {
			...
			Logger::info( 'Private key loaded (length=' . strlen( $privateKey ) . ' chars).' );
			if ( !$sftp->login( $username, $privateKey ) ) {
				$this->error( "SFTP login failed for $username@$host (private key)" );
			}
		} else {
			$this->error( 'No authentication available: provide --passwordfile, config sftp.password, or config sftp.private_key.' );
		}

		$this->logNegotiatedAlgorithms( $sftp );

		$remoteEntries = $this->listRemoteFilesWithMtime( $sftp, $remoteDir );
		...
		foreach ( $candidates as $entry ) {
			...
			if ( $deleteRemote ) {
				...
				if ( $remoteMTime <= 0 ) {
					$remoteMTime = $sftp->filemtime( $remotePath );
				}
			...
			$ok = $sftp->get( $remotePath, $tmpPath );
			if ( !$ok ) {
			...
			// Reject empty downloads (Python parity).
			clearstatcache( true, $tmpPath );
			...
			if ( $rejectEmpty && (int)$finalSize === 0 ) {
				if ( !unlink( $tmpPath ) ) {
	...
	/**
	 * Configure SSH algorithms for best interoperability.
	 *
	 * phpseclib v2 does not support several modern KEX algorithms (eg curve25519 / sha256 / sha512 KEX).
	 * We keep a "candidate" list for future v3 support, but only pass v2-supported algorithms to phpseclib v2
	 ...
	 */
	/**
	 * Configure modern SSH algorithms (phpseclib v3).
	 *
	 * @param SFTP $sftp
	 * @return void
	 */
	private function configureSshAlgorithms( SFTP $sftp ): void {
	...
	/**
	 * Log negotiated SSH algorithms (v3).
	 *
	 * @param SFTP $sftp
	 * @return void
	 */
	private function logNegotiatedAlgorithms( SFTP $sftp ): void {
		$ssh2 = $sftp->getSSH2();
		$algorithms = $ssh2 ? $ssh2->getAlgorithmsNegotiated() : null;
	 ...
	 */
	private function listRemoteFilesWithMtime( SFTP $sftp, string $remoteDir ): array {
		$entries = [];

		$raw = $sftp->rawlist( $remoteDir );
		if ( is_array( $raw ) ) {
		...
		foreach ( $list as $name ) {
			...
			$mtime = $sftp->filemtime( $remoteDir . '/' . $name );
			$entries[] = [
				'name' => (string)$name,
	...
	/**
	 * Load YAML config from either an explicit --config path or a --configname.
	 *
	 * @return array<string,mixed>
	 */
	private function loadYamlConfig(): array {
		$configPath = $this->normalizePath( (string)$this->getOption( 'config' ) );
		$configName = trim( (string)$this->getOption( 'configname' ) );
	...
	private function findConfigPathByName( string $name ): string {
		if ( $name === '' ) {
			$this->error( 'configname must not be empty' );
		}
	 ...
	 * @return mixed
	 */
	private function getFromConfig( string $path, $default = null ) {
		$current = $this->config;
	 ...
	 * @return mixed
	 */
	private function chooseOptionOrConfig( string $optName, array $config, string $configPath, $default = null ) {
	...
	/**
	 * Resolve password authentication from either --passwordfile or config sftp.password.
	 *
	 * @param array<string,mixed> $config
	 * @return string|null
	 */
	private function readPasswordFromConfigOrFile( array $config ): ?string {
		$passwordFile = (string)$this->getOption( 'passwordfile' );
		if ( $passwordFile !== '' ) {
	 ...
	 * @return mixed|null
	 */
	private function loadPrivateKey( array $config ) {
		...
		return $this->loadPrivateKeyFromConfig( $config );
	}
	 ...
	 * @return mixed|null
	 */
	private function loadPrivateKeyFromConfig( array $config ) {
	 ...
	 *
	 * Expected config format: "ssh-rsa <base64>".
	 *
	 * @param SFTP $sftp
	 * @param array<string,mixed> $config
	 * @param string $host
	 * @return void
	 */
	private function verifyPinnedHostKeyOrError( SFTP $sftp, array $config, string $host ): void {
		...
		$parts = preg_split( '/\s+/', $pinned );
		if ( !$parts || count( $parts ) < 2 ) {
			$this->error( 'Invalid sftp.host_key format in config (expected "ssh-rsa <base64>").' );
		}

		$pinnedType = $parts[0];
		$pinnedB64 = $parts[1];

		$server = $sftp->getServerPublicHostKey();
		if ( $server === false || $server === null || $server === '' ) {
			$this->error( "Could not read server host key from $host" );
		}

		$serverB64 = base64_encode( $server );

		if ( $serverB64 !== $pinnedB64 ) {
			$this->error( "Host key mismatch for $host (type $pinnedType). Refusing to connect." );
		}

		Logger::info( "Host key verified for $host ($pinnedType)" );
	}
	 ...
	 * @return string[]
	 */
	private function normalizeExtraPaths( $extraPaths ): array {
	 ...
	 *
	 * Requires that incomingdir ends with 'incoming' for safety.
	 *
	...
	private function inferSiblingDir( string $incomingDir, string $siblingName ): string {
		...
		if ( $leaf !== 'incoming' ) {
			$this->error(
				"incomingdir must end with 'incoming' to infer sibling directories; got: $incomingDir"
			);
		}
